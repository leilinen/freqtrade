...
def _build_ohlcv_df(n: int):
    """Build a synthetic OHLCV df with `n` rows using freqtrade columns."""
    import numpy as np
    import pandas as pd
...
def test_rpc_quote_pair_not_in_whitelist(default_conf, mocker) -> None:
    mocker.patch("freqtrade.rpc.telegram.Telegram", MagicMock())
    freqtradebot = get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)

    result = rpc._rpc_quote("SCAM/BTC", None, 20)
    assert result["error"] is not None
    assert "not in the bot whitelist" in result["error"]
    assert result["chart_png"] is None


def test_rpc_quote_no_cached_data(default_conf, mocker) -> None:
    import pandas as pd
    ...
    # Pair is whitelisted but dataprovider returns empty df.
    mocker.patch.object(
        freqtradebot.dataprovider, "get_pair_dataframe", return_value=pd.DataFrame()
    )

    result = rpc._rpc_quote("ETH/BTC", None, 20)
    assert result["error"] is not None
    assert "No cached data" in result["error"]
    assert result["chart_png"] is None


def test_rpc_quote_success(default_conf, mocker) -> None:
    mocker.patch("freqtrade.rpc.telegram.Telegram", MagicMock())
    freqtradebot = get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)

    df = _build_ohlcv_df(50)
    mocker.patch.object(freqtradebot.dataprovider, "get_pair_dataframe", return_value=df)
    mocker.patch(
        "freqtrade.rpc.chart_builder.generate_candlestick_chart",
        return_value=b"\x89PNG-fake",
    )

    result = rpc._rpc_quote("ETH/BTC", None, 20)
    assert result["error"] is None
    assert result["chart_png"] == b"\x89PNG-fake"
    assert result["pair"] == "ETH/BTC"
    assert result["num_candles"] == 20
    assert "O:" in result["ohlcv_text"]
    assert "C:" in result["ohlcv_text"]


def test_rpc_quote_chart_import_error(default_conf, mocker) -> None:
    mocker.patch("freqtrade.rpc.telegram.Telegram", MagicMock())
    freqtradebot = get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)
    ...
    mocker.patch(
        "freqtrade.rpc.chart_builder.generate_candlestick_chart",
        side_effect=ImportError("mplfinance not installed"),
    )

    result = rpc._rpc_quote("ETH/BTC", None, 20)
    assert result["error"] is not None
    assert "deps not installed" in result["error"].lower()
    assert result["chart_png"] is None


def test_rpc_quote_timeframe_override(default_conf, mocker) -> None:
    mocker.patch("freqtrade.rpc.telegram.Telegram", MagicMock())
    freqtradebot = get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)

    df = _build_ohlcv_df(30)
    mocker.patch.object(freqtradebot.dataprovider, "get_pair_dataframe", return_value=df)
    mocker.patch(
        "freqtrade.rpc.chart_builder.generate_candlestick_chart",
        return_value=b"\x89PNG-fake",
    ...
    result = rpc._rpc_quote("ETH/BTC", "1h", 10)
    assert result["error"] is None
