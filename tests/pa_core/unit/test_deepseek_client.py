...
from pa_core.settings import AIProviderSettings
from pa_core.llm.deepseek_client import (
    DeepSeekClient,
    AIReply,
    AIUsage,
    CancelledError,
...
def test_chat_does_not_send_forbidden_params():
    ...
    with patch("pa_core.llm.deepseek_client._OpenAI", mock_openai):
...
def test_sensenova_thinking_uses_enabled_disabled_not_adaptive():
    ...
    from pa_core.llm.deepseek_client import _resolve_thinking_params
...
def test_packy_hoists_system_message_to_extra_body():
    from pa_core.llm.deepseek_client import _prepare_chat_messages
...
@pytest.mark.skip(reason="pre-existing upstream failure (PA_Agent 1090a5b, no CI there); reproduced against original code")
...
def test_chat_no_plaintext_key_in_logs(caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="pa_core.llm.deepseek_client"):
        with patch("pa_core.llm.deepseek_client._OpenAI", mock_openai):
            client.chat([{"role": "user", "content": "hi"}])
...
def test_openclaw_agent_request_includes_tool_choice_none() -> None:
    # Migration note: QClaw/WorkBuddy agent routes are dropped in pa_core, so
    # the request extra is always {} now (upstream asserted tool_choice: none
    # with qclaw detection patched on).
    settings = _make_settings()
    settings.model = "openclaw"
    settings.base_url = "http://127.0.0.1:58579/v1"
    assert _openclaw_agent_request_extra(settings) == {}


def test_stream_chat_passes_tool_choice_none_for_openclaw() -> None:
    # Migration note: openclaw_* models now fail fast in DeepSeekClient (no
    # Cursor/TRAE/Qoder routes exist in pa_core); upstream asserted the request
    # carried tool_choice=none via the QClaw agent route.
    settings = _make_settings()
    settings.model = "openclaw"
    settings.base_url = "http://127.0.0.1:58579/v1"
    settings.thinking = False
    ...
    with patch("pa_core.llm.deepseek_client._OpenAI", MagicMock()):
