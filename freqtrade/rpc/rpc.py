...
class RPC:
    ...
    def _rpc_quote(self, pair: str, timeframe: str | None, num_candles: int) -> dict[str, Any]:
        """Build a /quote response: recent OHLCV chart PNG + compact text table.

        :param pair: Pair symbol, must be in the bot's current whitelist.
        :param timeframe: Timeframe string (e.g. '5m', '1h'). None falls back to
                          the configured strategy timeframe.
        :param num_candles: How many most-recent candles to plot and summarize.
        :return: dict with keys:
                 - pair, timeframe, num_candles
                 - chart_png: PNG bytes or None on error
                 - ohlcv_text: compact OHLCV text or empty string on error
                 - error: None on success, error message string otherwise
        """
        from freqtrade.rpc.chart_builder import generate_candlestick_chart

        timeframe = timeframe or self._config["timeframe"]

        whitelist = self._freqtrade.dataprovider.current_whitelist()
        if pair not in whitelist:
            return {
                ...
                "ohlcv_text": "",
                "error": (
                    f"`{pair}` is not in the bot whitelist. "
                    f"Whitelisted pairs: {', '.join(whitelist[:10])}"
                    + (" ..." if len(whitelist) > 10 else "")
                ),
            }

        df = self._freqtrade.dataprovider.get_pair_dataframe(pair, timeframe)
        if df is None or df.empty:
            return {
                ...
                "chart_png": None,
                "ohlcv_text": "",
                "error": f"No cached data for `{pair}` {timeframe}.",
            }

        try:
            chart_png = generate_candlestick_chart(pair, timeframe, df, num_candles)
        except ImportError:
            return {
                "pair": pair,
                "timeframe": timeframe,
                ...
                "chart_png": None,
                "ohlcv_text": "",
                "error": "Charting deps not installed. Run: pip install mplfinance matplotlib",
            }
        ...
        ohlcv_text = self._format_ohlcv_text(df.tail(num_candles))
    ...
    @staticmethod
    def _format_ohlcv_text(df: DataFrame) -> str:
        """Render the tail OHLCV rows as a compact Markdown-ish text table.

        Columns: time, O, H, L, C, V (precisions fixed for readability).
        """
        if df is None or df.empty:
            return ""
        lines = []
        for _, row in df.iterrows():
            ...
            lines.append(
                f"`{ts_str}` O:{row['open']:.4f} H:{row['high']:.4f} "
