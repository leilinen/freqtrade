...
from pa_core.llm.mimo_compat import (
    ReasoningCache,
    is_mimo_provider,
...
def _is_qclaw_openclaw_agent(settings: AIProviderSettings) -> bool:
    ...
    Migration note: pa_core drops the QClaw/Cursor/WorkBuddy/TRAE/Qoder agent
    routes — always False here. Kept as a stub so call sites stay unchanged.
    """
    return False
...
def _openclaw_agent_request_extra(settings: AIProviderSettings) -> dict[str, Any]:
    ...
    Migration note: always {} without the agent routes.
    """
    return {}
...
def _is_workbuddy_agent(settings: AIProviderSettings) -> bool:
    ...
    Migration note: always False without the WorkBuddy route.
    """
    return False
...
def _effective_api_model(settings: AIProviderSettings) -> str:
    ...
    Migration note: the WorkBuddy alias resolution is gone with the agent
    routes — the configured model id is used verbatim.
    """
...
class DeepSeekClient:
    ...
    def stream_chat(
        ...
        *,
        ...
        # Migration note: the per-connector openclaw_cs/twc/qc guards are replaced
        # by a plain prefix check — pa_core has no Cursor/TRAE/Qoder routes, so any
        # such model name is a misconfiguration.
        if _is_openclaw_agent_model(self._settings.model):
            raise RuntimeError(
                "模型 openclaw_* 需要 Cursor/TRAE/Qoder 专用路由，但 pa_core 只保留"
                " DeepSeek 兼容通道。请在配置中使用 deepseek 或其他 OpenAI 兼容模型。"
            )
