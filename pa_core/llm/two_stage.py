...
if TYPE_CHECKING:
    ...
    from pa_core.settings import Settings
    from pa_core.records.experience_reader import ExperienceReader
    from pa_core.records.pending_writer import PendingWriter

from pa_core.validation.json_validator import Ok, ValidationError
from pa_core.llm.validation_retry import validate_with_retry
from pa_core.data_structures import KlineFrame
from pa_core.records.schema import AnalysisRecord, RecordMeta
from pa_core.util.threading import CancelToken, OrchestratorEvent
...
def _enrich_stage2_validation_message(err: ValidationError, reply: Any) -> str:
    ...
    from pa_core.validation.provider_errors import (
    ...
    from pa_core.validation.validation_messages import format_validation_errors
...
def _build_empty_record(
    ...
    from pa_core.llm.decision_stance import normalize_stance
...
class TwoStageOrchestrator:
    ...
    def _validation_settings(self) -> Any:
        ...
        from pa_core.settings import ValidationSettings
    ...
    def submit(
        ...
        *,
        ...
        from pa_core.decision.decision_nodes import check_preflight_data
        ...
        if gate_result in ("wait", "unknown"):
            from pa_core.decision.decision_tree import build_stage2_gate_wait_response
    ...
    def _stream_chat_resilient(
        ...
        *,
        ...
        """Call stream_chat directly.

        Migration note: the QClaw/Cursor/WorkBuddy/TRAE-CN provider fallbacks
        and their retry loop are removed — pa_core talks to a single
        OpenAI-compatible endpoint, so connection errors propagate to the
        caller instead of switching providers mid-run.
        """
    ...
    def _is_network_error(exc: Exception) -> bool:
        ...
        from pa_core.llm.deepseek_client import CancelledError
