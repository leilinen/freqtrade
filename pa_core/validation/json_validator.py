...
class JsonValidator:
    ...
    def __init__(self, validation: Any = None) -> None:
        from pa_core.validation.schemas import STAGE1_SCHEMA, STAGE2_SCHEMA
    ...
    def validate(
        ...
        *,
        ...
        if stage == "stage1":
            from pa_core.validation.coherence_checks import auto_fix_bar_by_bar_types
            ...
            if getattr(self._validation, "stage1_coherence_checks", False):
                from pa_core.decision.decision_tree import validate_gate_result_consistency
                from pa_core.validation.coherence_checks import (
            ...
            if getattr(self._validation, "trace_semantic_checks", False):
                from pa_core.validation.trace_semantic_checks import validate_trace_semantics
        ...
        if stage == "stage2":
            ...
            if getattr(self._validation, "stage2_coherence_checks", False):
                from pa_core.decision.decision_tree import validate_stage2_trace_consistency
                from pa_core.validation.coherence_checks import validate_stage2_coherence
            ...
            if getattr(self._validation, "trace_semantic_checks", False):
                from pa_core.validation.trace_semantic_checks import (
    ...
    def _check_trade_metrics(
        ...
        *,
        ...
        from pa_core.util.trade_metrics import validate_order_trade_metrics
    ...
    def _check_next_cycle_prediction(obj: dict) -> list[str]:
        ...
        """
        from pa_core.decision.cycle_enums import CYCLE_ENUM, CYCLE_ORDER
